...
///  */
#[repr(align(4))]
pub struct AlignedRxBuf(pub [u8; 8192 + 16]);
pub static mut RX_BUFFER: AlignedRxBuf = AlignedRxBuf([0; 8192 + 16]);
use crate::{
    print,
...
#[repr(align(4))]
pub struct AlignedTxBuf(pub [[u8; 1792]; 4]);
pub static mut TX_BUFFERS: AlignedTxBuf = AlignedTxBuf([[0; 1792]; 4]);
...
pub fn rtl8139_handler(io_addr: u16, rx_buffer: &[u8]) {
    ...
    unsafe {
        ...
        if (status & ROK) != 0 {
            if let Some(packet) = receive_packet(io_addr, rx_buffer) {
                crate::net::increment_rx();

                if let Some((eth, payload)) = crate::net::ethernet::parse(&packet) {
                    match payload {
                        crate::net::ethernet::EtherPayload::Arp(arp_data) => {
                            let our_mac = crate::net::get_mac_address()
                                .unwrap_or([0x52, 0x54, 0x00, 0x12, 0x34, 0x56]);
                            let our_ip = crate::net::get_ip_address().unwrap_or([10, 0, 2, 15]);
                            if let Some(reply) = crate::net::arp::handle(arp_data, our_mac, our_ip)
                            {
                                let frame =
                                    crate::net::ethernet::build(eth.src, our_mac, 0x0806, &reply);
                                transmit_packet(io_addr, &frame);
                            }
                        }
                        crate::net::ethernet::EtherPayload::Ipv4(ip_data) => {
                            if let Some((ip_hdr, ip_payload)) = crate::net::ipv4::parse(ip_data) {
                                match ip_payload {
                                    crate::net::ipv4::IpPayload::Icmp(icmp) => {
                                        crate::net::icmp::handle_icmp_packet(ip_hdr.src, icmp);
                                    }
                                    crate::net::ipv4::IpPayload::Tcp(tcp) => {
                                        crate::net::tcp::handle_packet(ip_hdr.src, tcp);
                                    }
                                    _ => {}
                                }
                            }
...
pub fn transmit_packet(io_addr: u16, data: &[u8]) {
    unsafe {
        ...
        TX_BUFFERS.0[slot][..data.len()].copy_from_slice(data);
